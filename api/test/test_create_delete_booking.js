...
var data = require('../page/test_data');
...
const testCaseBooking = {
  positive: {
    ...
    getBooking:
      'As an User, I should be able to retrieve booking details using booking id',
  },
  negative: {
    invalidData:
      'As an User, I should not be able to create booking if the data provided is invalid',
  },
};
...
describe(`Automation API using Mocha and Chai`, () => {
  ...
  describe('Test Case for Get Booking', () => {
    it(`@get ${testCaseBooking.positive.getBooking}`, async () => {
      const response = await page.getBooking(bookingID, token);
